...
<template>
  <div class="max-w-lg mx-auto my-auto p-6 bg-white rounded-lg shadow-lg">
    ...
    <template v-if="localProperty">
      ...
      <div v-else-if="localProperty.owner && isOwnerDetailsVisible" class="mt-4 text-gray-900">
        ...
        <div class="flex flex-col items-center space-y-3">
          ...
          <p>
            ...
            {{ localProperty.owner.name }}
          </p>
          <p>
            <span class="font-semibold">Phone:</span>
            {{ localProperty.owner.phone }}
          </p>
          <p>
            <span class="font-semibold">Email:</span>
            {{ localProperty.owner.email }}
          </p>
          <p>
            <span class="font-semibold">Address:</span>
            {{ localProperty.owner.address }}
          </p>
        </div>
      </div>
    </template>
  ...
  methods: {
    async fetchProperty() {
      ...
      try {
        ...
        if (data) {
          this.localProperty = { id: this.id, ...data.data, owner: data.owner };
        } else {
